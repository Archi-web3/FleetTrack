...
export type ProfileMatrix = Record<string, RolePermissions>;
...
export class PermissionsService {
  ...
  private defaultMatrix: ProfileMatrix = {
    SuperAdmin: {
      ...
      flotte_generateurs: { view_menu: true, manage: true },
    ...
    Superviseur: {
      ...
      mobile_chauffeur: {
        ...
        start_trip: false,
        end_trip: false,
        fill_daily_log: false,
        report_incident: false,
      },
      ...
      flotte_generateurs: { view_menu: false, manage: false },
          ...
          if (!parsed[profile]) {
            ...
            for (const module of Object.keys(this.defaultMatrix[profile])) {
              if (!parsed[profile][module]) {
                parsed[profile][module] = JSON.parse(
                  JSON.stringify(this.defaultMatrix[profile][module]),
                );
                hasChanges = true;
              } else {
