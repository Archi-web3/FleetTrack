...
export class MaintenanceTrackingComponent implements OnInit, AfterViewInit {
  private trackingService = inject(MaintenanceTrackingService);
  ...
  getStatusLabel(statusCode: string): string {
    switch (statusCode) {
      case 'ok':
        return this.translate.instant('MAINTENANCE.STATUS.UP_TO_DATE');
      case 'proche':
        return this.translate.instant('MAINTENANCE.STATUS.CLOSE');
      case 'retard':
        return this.translate.instant('MAINTENANCE.STATUS.OVERDUE');
      case 'critique':
        return this.translate.instant('MAINTENANCE.STATUS.CRITICAL');
      default:
        return this.translate.instant('COMMON.UNKNOWN') || 'Inconnu';
    }
  }
  ...
  formatEcart(ecartKm: number | null): string {
    ...
    if (ecartKm > 0)
      return `${this.translate.instant('MAINTENANCE.IN')} ${ecartKm} ${this.translate.instant('MAINTENANCE.KM')}`;
  ...
  loadMaintenanceSlots() {
    ...
    this.mouvementService.getMouvements().subscribe({
      next: (data: any) => {
        ...
        this.maintenanceSlots.sort(
          (a: any, b: any) => new Date(b.dateDepart).getTime() - new Date(a.dateDepart).getTime(),
        );
  ...
  deleteSlot(slot: any) {
    if (
      confirm(
        `Confirmer la suppression de la maintenance "${slot.maintenanceType}" pour ${slot.vehicule?.immatriculation} ?`,
      )
    ) {
