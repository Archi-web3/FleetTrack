...
export class WeeklyChecklistTracker implements OnInit {
  ...
  sortVehicles() {
    ...
    this.vehicles.sort((a, b) => {
      ...
      return (
        (priority[statusA as keyof typeof priority] || 2) -
        (priority[statusB as keyof typeof priority] || 2)
      );
    });
  }
  ...
  getStatusLabel(status: string): string {
    switch (status) {
      case 'ok':
        return this.translate.instant('WEEKLY_CHECKLIST.STATUS_OK');
      case 'late':
        return this.translate.instant('WEEKLY_CHECKLIST.STATUS_LATE');
      case 'never':
        return this.translate.instant('WEEKLY_CHECKLIST.STATUS_NEVER');
      default:
