...
export class GestionLieuxComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.dataSource.sortingDataAccessor = (item, property) => {
      switch (property) {
        case 'pays':
          return item.pays?.nom;
        case 'base':
          return item.base?.nom;
        case 'securite':
          return item.niveauSecurite;
        default:
  ...
  openLieuModal(lieu?: any): void {
    if (lieu) {
      ...
      this.newLieu = {
        ...
        coordonnees: {
          latitude: lieu.coordonnees?.latitude || 0,
          longitude: lieu.coordonnees?.longitude || 0,
        },
    ...
    this.dialog.open(this.lieuFormDialog, {
      width: '800px',
