...
export class GenerateurDetailComponent implements OnInit {
  ...
  loadData() {
    ...
    this.generateursService.getGenerateur(this.generateurId).subscribe((gen) => {
      ...
      this.generateursService
        .getLogbooks(this.generateurId)
        .subscribe((logs) => (this.logbooks = logs));
      this.generateursService
        .getInterventions(this.generateurId)
        .subscribe((intervs) => (this.interventions = intervs));
