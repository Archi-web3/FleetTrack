...
export class GenerateurFormComponent implements OnInit {
  ...
  loadReferentials() {
    ...
    const selectedPaysId =
      localStorage.getItem('selectedCountry') || this.authService.getUserPaysId() || undefined;
  ...
  onError(err: any) {
    ...
    console.error(err);
    this.snackBar.open(
      'Erreur lors de la sauvegarde (Vérifiez le Numéro de Série unique)',
      'Fermer',
      { duration: 5000 },
    );
  }
