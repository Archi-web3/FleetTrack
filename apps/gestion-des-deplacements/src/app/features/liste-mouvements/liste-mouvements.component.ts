...
export class ListeMouvementsComponent implements OnInit, OnDestroy {
  ...
  showCompleted = false; // Par défaut, masquer les mouvements terminés
  ...
  ngOnDestroy(): void {
    // <<< NOUVEAU : Pour nettoyer l'abonnement
  ...
  completeMouvement(mouvement: any): void {
    if (
      confirm(
        `Confirmer que le voyage "${mouvement.objectif}" a bien été effectué ?\nCela clôturera le mouvement.`,
      )
    ) {
