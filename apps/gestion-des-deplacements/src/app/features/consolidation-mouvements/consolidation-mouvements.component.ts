...
export class ConsolidationMouvementsComponent implements OnInit {
  ...
  loadDataForConsolidation(): void {
    forkJoin({
      ...
      ({ mouvements, vehicules, utilisateurs }) => {
        ...
        const chauffeurs = utilisateurs.filter(
          (u: any) => u.profil && u.profil.toLowerCase() === 'chauffeur',
        );
        ...
        // Filtrer les mouvements "validés" OU "en attente validation sécurité" OU "en attente" (Workflow Parallèle)
        // IMPORTANT: Exclure les mouvements "regroupé" (mouvements enfants) ET Aérien/Maritime
        this.mouvementsValides = mouvements.filter(
          (m) =>
            (m.statut === 'validé' ||
        ...
        this.mouvementsConsolides = mouvements.filter(
          (m) =>
            (m.statut === 'validé' ||
              m.statut === 'pris en charge' ||
              m.statut === 'en cours' ||
              m.statut === 'regroupé' ||
              m.statut === 'terminé') &&
            m.stops &&
  ...
  updateMapEvents(mouvementsList: any[]): void {
    this.mapEvents = mouvementsList
      .filter(
        (mouvement) =>
          mouvement.stops &&
          mouvement.stops.length > 0 &&
          mouvement.stops.every(
            (stop: any) =>
  ...
  onToggleMapDisplay(event: any, mouvement: any): void {
    if (event.target.checked) {
      ...
      this.mouvementsToShowOnMap = this.mouvementsToShowOnMap.filter(
        (m) => m._id !== mouvement._id,
      );
    }
    this.updateMapEvents(this.mouvementsToShowOnMap);
  }
  ...
  regrouperMouvementsSelectionnes(): void {
    ...
    const allPassagersIds = Array.from(
      new Set(this.mouvementsToRegroup.flatMap((m) => m.passagers.map((p: any) => p._id))),
    );
      ...
      (newRegroupedMouvement) => {
        ...
        const updatePromises = this.mouvementsToRegroup.map((m) => {
          return firstValueFrom(
            this.mouvementService.updateMouvement(m._id, {
              statut: 'regroupé',
  ...
  updateConsolidationMap(): void {
    ...
    const validStops = this.consolidationStops.filter(
      (stop) =>
        stop.lieu?.coordonnees &&
        typeof stop.lieu.coordonnees.latitude === 'number' &&
        typeof stop.lieu.coordonnees.longitude === 'number',
  ...
  removeStop(index: number): void {
    ...
    if (
      confirm(
        `Voulez-vous vraiment supprimer l'étape "${this.consolidationStops[index].lieu.nom}" ?`,
      )
    ) {
  ...
  getFinalDestination(mouvement: any): string {
    if (
      mouvement.stops &&
      mouvement.stops.length > 0 &&
      mouvement.stops[mouvement.stops.length - 1].lieu
    ) {
  ...
  getLastDateArrivee(mouvement: any): string {
    if (
      mouvement.stops &&
      mouvement.stops.length > 0 &&
      mouvement.stops[mouvement.stops.length - 1].dateArrivee
    ) {
  ...
  cleanGhosts(): void {
    if (
      !confirm(
        '⚠️ ATTENTION : Cette action va scanner et supprimer tous les mouvements "regroupés" orphelins (dont le parent n\'existe plus). Êtes-vous sûr de vouloir continuer ?',
      )
    ) {
      return;
    }
