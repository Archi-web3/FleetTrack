...
export class ValidationMouvementsComponent implements OnInit {
  ...
  loadMouvementsPourValidation(): void {
    if (!this.userProfile) {
      ...
      console.warn(
        '⚠️ [VALIDATION] Profil utilisateur non chargé. Impossible de filtrer les mouvements pour validation.',
      );
      return;
    }
    ...
    this.mouvementService.getMouvements().subscribe(
      (data) => {
        ...
        if (
          this.mouvementsPourValidationLogistique.length === 0 &&
  ...
  validerMouvement(
    ...
    type: 'security' | 'logistics' = 'security',
  ): void {
        ...
        (error) => {
          ...
          if (error.status === 403) {
            alert(
              '⛔ REFUSÉ : ' +
                (error.error?.message ||
                  'Votre niveau de validation est insuffisant pour ce trajet à risque.'),
            );
          } else {
  ...
  refuserMouvement(mouvementId: string): void {
    ...
    this.mouvementService
      .updateMouvement(mouvementId, { statut: 'refusé', motifRefus: motif })
      .subscribe(
        (response) => {
        ...
        (error) => {
          ...
          if (error.status === 403)
            alert("Accès refusé. Vous n'êtes pas autorisé à refuser ce mouvement.");
  ...
  hasUserValidated(mouvement: any): boolean {
    ...
    const approval = mouvement.securityApprovals.find(
      (a: any) => (a.validator._id || a.validator) === userId, // Gérer si populated ou non
    );
