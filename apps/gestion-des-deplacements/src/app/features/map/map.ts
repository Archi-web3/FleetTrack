...
export class MapComponent implements OnInit {
  ...
  updateMovementsToDisplay(): void {
    if (this.selectedTripIds.size > 0) {
      ...
      this.movementsToDisplay = this.filteredMouvements.filter((m) =>
        this.selectedTripIds.has(m.id),
      );
    } else {
    ...
    this.isAllSelected =
      this.filteredMouvements.length > 0 &&
      this.selectedTripIds.size === this.filteredMouvements.length;
  }
  ...
  transformToMapMouvement(trip: any): any {
    ...
    return {
      ...
      vehiculeName: trip.vehicule
        ? `${trip.vehicule.marque || ''} ${trip.vehicule.modele || ''} (${trip.vehicule.immatriculation || '?'})`
        : 'Véhicule Inconnu',
