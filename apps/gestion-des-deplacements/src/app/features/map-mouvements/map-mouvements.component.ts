...
export class MapMouvementsComponent implements OnInit, OnChanges, AfterViewInit {
  ...
  showPlanned = true;
  showReal = true;
  selectedMode = 'ALL'; // Nouveau filtre Mode
  transportModes = ['ALL', 'ROUTIER', 'AERIEN', 'MARITIME'];

  private map!: L.Map;
  private markersMap: Record<string, L.Marker> = {}; // Map trip ID to Marker for fast access
  ...
  private updateMapMarkers(): void {
    ...
    const filteredMouvements = this.mouvements.filter((m) => {
      ...
      if (this.selectedMode === 'AERIEN')
        return (
          lowerTitle.includes('avion') || lowerTitle.includes('vol') || lowerTitle.includes('air')
        );
      if (this.selectedMode === 'MARITIME')
        return (
          lowerTitle.includes('bateau') ||
          lowerTitle.includes('mer') ||
          lowerTitle.includes('navire')
        );
      if (this.selectedMode === 'ROUTIER')
        return (
          !lowerTitle.includes('avion') &&
    ...
    filteredMouvements.forEach((mouvement) => {
        ...
        mouvement.stops.forEach((stop, index) => {
          if (stop.lat && stop.lng) {
            ...
            const marker = L.marker([stop.lat, stop.lng], {
              ...
              `<b>${index === 0 ? 'Départ Planifié' : 'Arrivée Planifiée'}</b><br>${mouvement.vehiculeName || 'Véhicule Inconnu'} <b>[${mouvement.vehiculeCode || '?'}]</b><br>${stop.nom}<br><i>${mouvement.title}</i>`,
            );
      ...
      if (this.showReal && mouvement.gpsTrace && mouvement.gpsTrace.length > 1) {
        ...
        const startMarker = L.circleMarker(gpsLatLngs[0], {
          radius: 6,
          fillOpacity: 1,
          color: '#2e7d32',
          fillColor: '#4caf50',
          ...
          .bindPopup(
            `<b>Départ Réel GPS</b><br>${mouvement.vehiculeName || ''} <b>[${mouvement.vehiculeCode || '?'}]</b><br>${mouvement.title}`,
          )
          .addTo(this.map!);
        this.realTraceLayers.push(startMarker);

        const endMarker = L.circleMarker(gpsLatLngs[gpsLatLngs.length - 1], {
          radius: 6,
          fillOpacity: 1,
          color: '#c62828',
          fillColor: '#ef5350',
          ...
          .bindPopup(
            `<b>Dernière Position Connue</b><br>${mouvement.vehiculeName || ''} <b>[${mouvement.vehiculeCode || '?'}]</b><br>${mouvement.title}`,
          )
          .addTo(this.map!);
