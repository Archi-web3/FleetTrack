...
export class GeneralSettingsComponent implements OnInit {
  ...
  loadBrandSettings() {
    this.settingsService.getBrandSettings().subscribe((data) => {
      if (data) {
        ...
        if (!this.brandSettings.headerDisplayMode)
          this.brandSettings.headerDisplayMode = this.brandSettings.displayMode;
        if (!this.brandSettings.heroDisplayMode)
          this.brandSettings.heroDisplayMode = this.brandSettings.displayMode;
      }
    });
  }
    ...
    this.settingsService.saveBrandSettings(this.brandSettings).subscribe({
      next: () => {
        ...
        document.documentElement.style.setProperty(
          '--primary-color',
          this.brandSettings.primaryColor,
        );
      },
      error: (err) => {
        console.error(err);
  ...
  emailNotifications: any[] = [
    {
      id: 'req_created',
      name: '1. Nouvelle demande (vers Logistique)',
      desc: "Envoyé aux logisticiens lors d'une nouvelle requête",
      workflowDesc: "Cette notification est le point de départ. Elle informe l'équipe logistique qu'une nouvelle demande vient d'être soumise par un utilisateur. Le logisticien doit se connecter pour assigner un véhicule et un chauffeur.",
      enabled: true,
      useMatrixRecipients: true,
      recipientProfiles: ['Logisticien', 'SuperAdmin'],
      recipientUsers: [],
      subject: '[FleetTrack] Nouvelle demande de mouvement : {{movementId}}',
      body: 'Bonjour,\n\nUne nouvelle demande de mouvement a été soumise par {{user}}.\n\nMerci de vous connecter pour lui assigner un chauffeur et un véhicule.\n\nLien : {{link}}',
    },
    {
      id: 'log_validated',
      name: '2. Consolidation Logistique (vers Sécurité)',
      desc: 'Envoyé à la sécurité après assignation du véhicule et du chauffeur.',
      workflowDesc: "Cette notification s'active uniquement lorsque le logisticien a consolidé le mouvement (chauffeur et véhicule attribués). Elle informe la sécurité que la demande est prête pour leur validation finale.",
      enabled: true,
      useMatrixRecipients: true,
      recipientProfiles: ['Superviseur Sécurité'],
      recipientUsers: [],
      subject: '[FleetTrack] Validation Logistique : {{movementId}}',
    ...
    {
      id: 'sec_request',
      name: 'Demande de Validation Sécurité',
      desc: 'Envoyé aux validateurs de sécurité pour demander leur approbation',
      workflowDesc: 'Déclenchée après validation logistique (ou création) si le niveau de sécurité l\'exige. Le valideur de sécurité reçoit ce message pour examiner la demande.',
      enabled: true,
      useMatrixRecipients: true,
      recipientProfiles: ['Superviseur Sécurité', 'Admin'],
      recipientUsers: [],
      subject: '[FleetTrack] 🛡️ Validation Requise : {{movementId}}',
      body: "Bonjour,\n\nUn mouvement nécessite votre approbation sécurité.\nDemandeur : {{user}}\nRéférence : {{movementId}}\n\nMerci de vous connecter pour valider la demande.\n\nLien : {{link}}",
    },
    {
      id: 'sec_validated',
      ...
      recipientUsers: [],
      subject: '[FleetTrack] Validation Sécurité Accordée',
    ...
    },
    {
      ...
      recipientProfiles: [],
      recipientUsers: [],
      subject: '[FleetTrack] Mouvement Confirmé',
    ...
    },
    {
      ...
      recipientProfiles: ['Logisticien'],
      recipientUsers: [],
      subject: '[FleetTrack] Mouvement Annulé',
    ...
    },
    {
      ...
      recipientProfiles: ['Logisticien'],
      recipientUsers: [],
      subject: '[FleetTrack] Alerte Maintenance : {{vehiclePlate}}',
  ...
  saveEmailSettings() {
    ...
    this.settingsService.saveEmailSettings(this.emailNotifications, key).subscribe({
      next: () => {
        ...
        this.snackBar.open("Paramètres d'emails et templates sauvegardés", 'OK', {
          duration: 2000,
        });
      },
      error: (err) => {
