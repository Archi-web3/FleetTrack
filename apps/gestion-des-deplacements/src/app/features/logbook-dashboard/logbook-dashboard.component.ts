...
export class LogbookDashboardComponent implements OnInit, AfterViewInit {
  ...
  displayedColumnsIncidents: string[] = [
    'date',
    'type',
    'severity',
    'description',
  ...
  private _loadVehicleData(): void {
    ...
    this.logbookService
      .getFuelsByVehicle(this.selectedVehiculeId)
      .subscribe((data) => (this.fuelDataSource.data = data));
    this.logbookService
      .getMaintenancesByVehicle(this.selectedVehiculeId)
      .subscribe((data) => (this.maintenanceDataSource.data = data));
    this.logbookService
      .getIncidentsByVehicle(this.selectedVehiculeId)
      .subscribe((data) => (this.incidentsDataSource.data = data));
  }
  ...
  deleteItem(type: string, id: string): void {
    ...
    switch (type) {
      case 'fuel':
        observable = this.logbookService.deleteFuel(id);
        break;
      case 'maintenance':
        observable = this.logbookService.deleteMaintenance(id);
        break;
      case 'incident':
        observable = this.logbookService.deleteIncident(id);
        break;
      case 'trips':
        observable = this.mouvementService.deleteMouvement(id);
        break;
    }
  ...
  saveItem(): void {
    ...
    switch (this.editingType) {
      case 'fuel':
        observable = this.logbookService.updateFuel(this.editingItem._id, this.editingItem);
        break;
      case 'maintenance':
        observable = this.logbookService.updateMaintenance(this.editingItem._id, this.editingItem);
        break;
      case 'incident':
        observable = this.logbookService.updateIncident(this.editingItem._id, this.editingItem);
        break;
    }
  ...
  openMap(trip: any): void {
    const getCoords = (lieu: any) => {
      ...
      if (typeof lieu.coordonnees === 'string') {
        ...
        lieu.coordonnees.latitude !== undefined &&
        lieu.coordonnees.longitude !== undefined
      ) {
        return {
    ...
    this.mapMouvementsData = [
      {
        ...
        vehiculeName: trip.vehicule?.marque
