...
export class PlanningMouvementsComponent implements OnInit {
      ...
      (data: any[]) => {
        ...
        this.currentWeekMovements = data.filter((m: any) => {
          ...
          const isInCurrentWeek =
            mouvementStart <= endOfCurrentWeek && mouvementEnd >= startOfCurrentWeek;
        ...
        for (let i = 0; i < this.events.length; i++) {
          for (let j = i + 1; j < this.events.length; j++) {
            ...
            if (
              e1.start < e2.end &&
              e1.end > e2.start &&
              e1.meta.statut !== 'annulé' &&
              e1.meta.statut !== 'refusé' &&
              e2.meta.statut !== 'annulé' &&
              e2.meta.statut !== 'refusé'
            ) {
  ...
  getEventTitle(mouvement: any): string {
    ...
    if (mouvement.type === 'maintenance') {
      const vehiculeInfo = mouvement.vehicule
        ? ` - ${mouvement.vehicule.acfCode || mouvement.vehicule.immatriculation}`
        : '';
    ...
    if (
      mouvement.statut === 'regroupé' &&
      mouvement.enfantsMouvements &&
      mouvement.enfantsMouvements.length > 0
    ) {
  ...
  getEventTooltip(mouvement: any): string {
    ...
    // Objectif
    lines.push(
      `📍 ${this.translate.instant('VALIDATION.OBJECTIVE')}: ${mouvement.objectif || 'Non spécifié'}`,
    );

    // Horaires
    // Simplified locale string usage for time
    const localeForTime = this.locale === 'en' ? 'en-US' : this.locale === 'es' ? 'es-ES' : 'fr-FR';
    const heureDepart = mouvement.dateDepart
      ? new Date(mouvement.dateDepart).toLocaleTimeString(localeForTime, {
          hour: '2-digit',
          minute: '2-digit',
        })
      : 'N/A';
    const heureArrivee = mouvement.dateArrivee
      ? new Date(mouvement.dateArrivee).toLocaleTimeString(localeForTime, {
          hour: '2-digit',
          minute: '2-digit',
        })
      : 'N/A';
    lines.push(
      `🕐 ${this.translate.instant('NEW_REQUEST.MISSION.DEPARTURE_TIME')}/${this.translate.instant('NEW_REQUEST.MISSION.ARRIVAL_TIME')}: ${heureDepart} → ${heureArrivee}`,
    );
    ...
    if (mouvement.vehicule) {
      lines.push(
        `🚗 Véhicule: ${mouvement.vehicule.marque} (${mouvement.vehicule.immatriculation})`,
      );
    }
    ...
    lines.push(
      `📊 ${this.translate.instant('MY_TRIPS.STATUS')}: ${this.translate.instant(statusKey)}`,
    );
  ...
  getStatusKey(status: string): string {
    ...
    switch (s) {
      ...
      case 'en attente validation sécurité':
        return 'En Attente Sécurité'; // TEMPORAIRE ou Key
      case 'terminé':
        return 'PLANNING.STATUS.COMPLETED';
      case 'annulé':
        return 'PLANNING.STATUS.CANCELLED';
      case 'refusé':
        return 'PLANNING.STATUS.CANCELLED';
      case 'en cours':
        return 'PLANNING.STATUS.IN_PROGRESS';
      case 'pris en charge':
        return 'PLANNING.STATUS.TAKEN';
      default:
        return status;
    }
  }
