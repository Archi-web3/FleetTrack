    ...
    },
    {
      ...
      modules: [
        ...
        },
        {
          ...
          permissions: [{ key: 'manage', label: 'Modifier la matrice' }],
        },
      ],
    },
  ];
      ...
      next: (users) => {
          ...
          .map((u) => {
            ...
            if (newLevel !== u.niveauValidationSecu) {
              return firstValueFrom(
                this.utilisateurService.updateUser(u._id, { niveauValidationSecu: newLevel }),
              );
            }
            return null;
          })
        ...
        if (updates.length > 0) {
          Promise.all(updates)
            .then(() => {
              this.snackBar.open(
                `Mise à jour automatique de ${updates.length} utilisateur(s) affecté(s).`,
                'OK',
                { duration: 4000 },
              );
            })
            .catch((err) => {
              ...
              this.snackBar.open('Erreur lors de la mise à jour des utilisateurs.', 'Fermer', {
                duration: 3000,
              });
            });
        }
