...
import {
  ...
  SecurityConfig,
  SecurityRule,
} from '../../core/services/security-config.service';
...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, TranslateModule, MatTooltipModule, MatIconModule, InfoBannerComponent],
...
export class SecurityMatrixComponent implements OnInit {
  private securityConfigService = inject(SecurityConfigService);
  ...
  ngOnInit(): void {
    this.canManageMatrix =
      this.perms.hasPermission('security_matrix', 'manage') ||
      localStorage.getItem('userProfile') === 'SuperAdmin';
  ...
  save(): void {
    ...
    configToSave.rules.forEach((rule: any) => {
      if (rule.mandatoryValidators) {
        rule.mandatoryValidators = rule.mandatoryValidators.map((v: any) =>
