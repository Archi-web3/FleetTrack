...
export class MonthlyReportComponent implements OnInit {
  ...
  async generateReport() {
    ...
    const selectedVehicles =
      vehicleId === 'all' ? this.vehicles : this.vehicles.filter((v) => v._id === vehicleId);
    ...
    for (const vehicle of selectedVehicles) {
      try {
        ...
        const maintenances = await this.logbookService
          .getMaintenancesByVehicle(vehicle._id)
          .toPromise();
        const checklists = await this.maintenanceService
          .getWeeklyChecklistHistory(vehicle._id, 100)
          .toPromise();

        // Filter movements for this vehicle
        const vehicleMovements = allMovements.filter(
          (m) => m.vehicule && (m.vehicule === vehicle._id || m.vehicule._id === vehicle._id),
        );
        ...
        const periodMaintenances =
          maintenances?.filter(
            (m) => m.date && new Date(m.date) >= startDate && new Date(m.date) <= endDate,
          ) || [];

        const periodChecklists =
          checklists?.filter((c) => {
        ...
        if (fuelsBefore.length > 0)
          eventsBefore.push(Math.max(...fuelsBefore.map((f) => f.mileage)));

        // Movements before period
        const movementsBefore = vehicleMovements.filter(
          (m) => m.dateArrivee && new Date(m.dateArrivee) < startDate && m.endMileage,
        );
        if (movementsBefore.length > 0)
          eventsBefore.push(Math.max(...movementsBefore.map((m) => m.endMileage)));
        ...
        if (eventsBefore.length > 0) {
          ...
          const startsDuring = periodMovements
            .map((m) => m.startMileage)
        ...
        if (periodFuels.length > 0)
          eventsDuring.push(Math.max(...periodFuels.map((f) => f.mileage)));
        if (periodMovements.length > 0)
          eventsDuring.push(Math.max(...periodMovements.map((m) => m.endMileage)));

        // End mileage is max of (StartMileage, MaxMileageDuringPeriod)
        const endMileage =
          eventsDuring.length > 0
            ? Math.max(startMileage, Math.max(...eventsDuring))
            : startMileage;
        ...
        const completionRate =
          periodChecklists.length > 0
            ? periodChecklists.reduce((sum, c) => sum + (c.tauxCompletion || 0), 0) /
              periodChecklists.length
            : 0;
        ...
        const rentalCost =
          vehicle.owner === 'Location' ? (vehicle.rentalCost || 0) * monthsDiff : 0;
        ...
        // --- DEBUG LOGGING ---
        if (
          vehicle.immatriculation.includes('CD-FG-HJ') ||
          vehicle.immatriculation.includes('MOB-002')
        ) {
