...
export class GestionVehiculesComponent implements OnInit, AfterViewInit {
  ...
  openVehiculeModal(vehicule?: any) {
    ...
    this.dialog.open(this.vehiculeFormDialog, {
      width: '800px',
  ...
  addVehicule(): void {
    ...
    this.vehiculeService.addVehicule(this.newVehicule).subscribe(
      ...
      (error) => {
        ...
        if (error.status === 403)
          alert("Accès refusé. Vous n'êtes pas autorisé à créer des véhicules.");
  ...
  selectVehicule(vehicule: any): void {
    ...
    if (!this.selectedVehicule.statut) {
      this.selectedVehicule.statut = this.selectedVehicule.enService
        ? 'En Service'
        : 'Hors Service';
    }
  ...
  updateVehicule(): void {
    ...
    this.vehiculeService.updateVehicule(this.selectedVehicule._id, this.selectedVehicule).subscribe(
      ...
      (error) => {
        ...
        if (error.status === 403)
          alert("Accès refusé. Vous n'êtes pas autorisé à modifier ce véhicule.");
  ...
  addEquipment(vehicule: any): void {
    const newCode =
      vehicule.equipementsDisplay && vehicule.equipementsDisplay.length > 0
    ...
    if (confirm('Êtes-vous sûr de vouloir supprimer ce véhicule ?')) {
      this.vehiculeService.deleteVehicule(id).subscribe(
        ...
        (error) => {
          ...
          if (error.status === 403)
            alert("Accès refusé. Vous n'êtes pas autorisé à supprimer des véhicules.");
  ...
  archiveVehicule(vehicule: any): void {
    if (
      confirm(
        'Voulez-vous vraiment archiver ce véhicule ? (Il ne sera plus sélectionnable pour les missions)',
      )
    ) {
  ...
  unarchiveVehicule(vehicule: any): void {
    const newKmStr = prompt(
      'Pour réactiver ce véhicule, veuillez confirmer son kilométrage ACTUEL (compteur) :',
    ...
    if (newKmStr !== null) {
      ...
      if (isNaN(newKm) || newKm < vehicule.kilometrage) {
        alert(
          "Kilométrage invalide. Le nouveau kilométrage doit être supérieur ou égal à l'ancien.",
        );
        return;
      }
