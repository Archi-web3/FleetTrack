...
export class WaiverComponent implements AfterViewInit {
  ...
  submitWaiver() {
    ...
    canvas.toBlob((blob) => {
      if (blob) {
        this.waiverService.createWaiver(this.visitorName, this.currentVehicleId!, blob).subscribe({
          next: () => {
          ...
          error: (err) => {
            console.error(err);
            this.snackBar.open("Erreur lors de l'envoi de la décharge", 'Réessayer', {
              duration: 3000,
            });
          },
        });
