...
export class ScheduledServiceComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (user && user['vehicule']) {
      this.selectedVehicleId =
        typeof user['vehicule'] === 'object' && user['vehicule'] !== null ? (user['vehicule'] as unknown as { _id?: string })._id || String(user['vehicule']) : String(user['vehicule']);
  ...
  getServiceTypeLabel(): string {
    ...
    switch (this.nextService.typeService) {
      case 'A':
        return 'Service A (Petit entretien)';
      case 'B':
        return 'Service B (Grand entretien)';
      case 'C':
        return 'Service C (Entretien majeur)';
      default:
  ...
  toggleTask(task: ServiceTask) {
    ...
    if (this.nextService) {
      this.maintenanceService
        .updateServiceTasks(this.nextService._id, this.nextService.taches as any)
        .subscribe({
          next: () => {},
          error: (err) => {
  ...
  completeService() {
    ...
    if (
      !confirm(
        'Confirmer que ce service a été effectué conformément aux recommandations constructeur ?',
      )
    ) {
      return;
    }
    ...
    this.maintenanceService.completeService(this.nextService._id, signature).subscribe({
      ...
      error: (error) => {
        ...
        alert(
          'Erreur lors de la validation du service : ' + (error.error?.message || error.message),
        );
      },
    });
  }
  ...
  openManual(numeroTache?: string) {
    ...
    if (match) {
      ...
      console.warn(
        `Manual Task Number '${numeroTache}' does not contain a valid page number. Defaulting to page 1.`,
      );
    }
