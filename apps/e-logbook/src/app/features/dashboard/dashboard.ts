...
import { VehicleRef, UserRef, BrandSettings, Movement } from '../../core/models/api.types';
...
export class DashboardComponent implements OnInit {
  ...
  private alertPoller = inject(AlertPollerService);
      ...
      this.maintenanceService.getNextService(this.selectedVehicle._id).subscribe({
        next: (service) => {
          if (service && (service.statut === 'Dû' || service.statut === 'En retard')) {
            this.alertsCount = 1;
            this.snackBar
              .open(`Service ${service.typeService} est ${service.statut}`, 'VOIR', {
                duration: 5000,
