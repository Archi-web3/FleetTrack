...
let MouvementsService = MouvementsService_1 = class MouvementsService {
    ...
    async create(createDto, user, forceConflict = false) {
        ...
        if (!finalPays && createDto.type !== 'maintenance') {
            throw new common_1.BadRequestException("Impossible de déterminer le pays pour ce mouvement. Veuillez sélectionner un pays de contexte ou préciser le lieu de départ.");
        }
        ...
        if (savedMouvement.statutSecurite === 'en attente') {
            ...
            if (emails.length > 0) {
                await this.mailService.sendTemplateEmail('sec_request', await savedMouvement.populate([
                    { path: 'vehicule' },
                    { path: 'stops.lieu' },
                    { path: 'demandeur' },
                ]), emails);
