...
export class PhotoService {
    ...
    async uploadPhoto(
        ...
        try {
            const result = await firstValueFrom(
                this.http.post<PhotoUploadResult>(`${this.apiUrl}/upload`, formData, { headers })
            );
            console.log('✅ [PhotoService] Upload réussi:', result.url);
            return result;
        } catch (error) {
    ...
    async deletePhoto(publicId: string): Promise<void> {
        ...
        console.log(`🗑️ [PhotoService] Suppression photo:`, publicId);

        try {
            await firstValueFrom(
                this.http.delete(`${this.apiUrl}/upload/${encodedPublicId}`, { headers })
            );
            console.log('✅ [PhotoService] Photo supprimée');
        } catch (error) {
